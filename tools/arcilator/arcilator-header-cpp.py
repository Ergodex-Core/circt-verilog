...
{% endif %}
  ...
  void eval() {
    ...
    {{ model.name }}_eval(storage);
#else
    {{ model.name }}_eval(storage.data());
#endif
  }
...
#ifdef ARC_USE_COMPILED_RUNTIME_LIB
    ValueChangeDump<{{ model.name }}Layout> vcd(os, storage);
#else
